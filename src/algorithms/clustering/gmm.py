...
class GMM:
    def __init__(self, df, num_clusters, eps=1e-6, max_iterations=1000):
        ...
        self.labels = np.floor_divide(np.arange(self.rows), self.rows / self.num_clusters)
    ...
    def run(self, with_steps):
        try:
            ...
            error.setText("Oops, singular matrix error. This data may not be good for this algorithm. Try to modify "
                          "params or choose different algorithm.")
    ...
    def e_step(self):
        self.prob_matrix = np.fromfunction(
            np.vectorize(self.calculate_prob),
            (self.rows, self.num_clusters),
            dtype=int
        )
        self.prob_matrix = self.prob_matrix/self.prob_matrix.sum(axis=1, keepdims=True)

    def calculate_prob(self, i, c):
        return self.pi_arr[c] * multivariate_normal.pdf(self.df.iloc[i], mean=self.mu_arr[c], cov=self.sigma_arr[c])
    ...
    def update_mu(self):
        sums = [np.sum((self.df.T * self.prob_matrix[:, c]).T, axis=0) for c in range(self.num_clusters)]
        sums = [sums[c] / (self.pi_arr[c] * self.rows) for c in range(self.num_clusters)]
    ...
    def log_likelihood(self):
        ...
        for i in range(self.rows):
            row_result = 0
            for c in range(self.num_clusters):
                row_result += self.pi_arr[c] * multivariate_normal.pdf(self.df.iloc[i], mean=self.mu_arr[c],
                                                                       cov=self.sigma_arr[c])
