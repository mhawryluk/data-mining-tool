...
class APrioriScatterPlot(FigureCanvasQTAgg):
    ...
    def plot_set(self, highlighted_set: tuple = None):
        ...
        mask = self._get_points_mask_set(
            set(highlighted_set) if highlighted_set is not None else None
        )
        self.sc = self.axes.scatter(
            list(compress(self.x_positions, mask)),
            list(compress(self.y_positions, mask)),
            color=self.HIGHLIGHTED_COLOR,
            s=10,
            zorder=2,
            label="containing the frequent set",
        )

        inverse_mask = list(map(operator.not_, mask))
        self.axes.scatter(
            list(compress(self.x_positions, inverse_mask)),
            list(compress(self.y_positions, inverse_mask)),
    ...
    def plot_rule(self, set_a: tuple, set_b: tuple):
        ...
        mask_a, mask_b, mask_a_b, mask_remaining = self._get_points_mask_rule(
            set(set_a), set(set_b)
        )
        self.axes.scatter(
            list(compress(self.x_positions, mask_a)),
            list(compress(self.y_positions, mask_a)),
            color=self.HIGHLIGHTED_RULE_A_COLOR,
            s=10,
            zorder=3,
            label="A",
        )
        ...
        self.axes.scatter(
            list(compress(self.x_positions, mask_b)),
            list(compress(self.y_positions, mask_b)),
            color=self.HIGHLIGHTED_RULE_B_COLOR,
            s=10,
            zorder=2,
            label="B",
        )
        ...
        self.axes.scatter(
            list(compress(self.x_positions, mask_a_b)),
            list(compress(self.y_positions, mask_a_b)),
            color=self.HIGHLIGHTED_RULE_A_B_COLOR,
            s=10,
            zorder=4,
            label="A and B",
        )
        ...
        self.axes.scatter(
            list(compress(self.x_positions, mask_remaining)),
            list(compress(self.y_positions, mask_remaining)),
            color=self.REGULAR_COLOR,
            s=10,
            zorder=1,
            label="~(A or B)",
    ...
    def _get_points_mask_set(self, highlighted_set: set = None):
        ...
        return [
            set(highlighted_set).issubset(transaction_set)
    ...
    def _get_points_mask_rule(self, set_a: set, set_b: set):
        return (
            ...
            [
                not set_a.issubset(transaction_set)
                and not set_b.issubset(transaction_set)
                for transaction_set in self.transaction_sets
            ],
        )
...
class APrioriGraphPlot(QWidget):
    ...
    def plot_rule(self, set_a, set_b):
        ...
        self.plot_graph(
            graph, nx.spring_layout(graph, k=0.15, iterations=20), set_a + set_b
        )
    ...
    def plot_graph(self, graph: nx.Graph, pos, labels):
        ...
        fig = go.Figure(
            ...
            layout=go.Layout(
                ...
                xaxis=dict(
                    showgrid=False, zeroline=False, showticklabels=False, range=[-2, 2]
                ),
                yaxis=dict(
                    showgrid=False, zeroline=False, showticklabels=False, range=[-2, 2]
                ),
    ...
    def show_placeholder(self):
        self.browser.setHtml(
            '<span style="color: grey">Click on a frequent set or an '
            "association rule in one of the tables to see visualization</span> "
        )
