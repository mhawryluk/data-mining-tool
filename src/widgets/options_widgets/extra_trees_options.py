from PyQt5.QtWidgets import QSpinBox, QLabel, QComboBox, QDoubleSpinBox
...
class ExtraTreesOptions(Options):
    def __init__(self):
        ...
        self.layout.addRow(QLabelWithTooltip("Column with labels:", "example"), self.label_name_box)
        ...
        self.layout.addRow(QLabelWithTooltip("Number of trees:"), self.forest_size_spinbox)

        self.features_number_spinbox = QSpinBox()
        self.features_number_spinbox.setMinimum(1)
        self.features_number_spinbox.setValue(1)
        self.layout.addRow(QLabelWithTooltip("Number of features to sample:"), self.features_number_spinbox)
        ...
        self.layout.addRow(QLabelWithTooltip("Minimum number of samples in child:"), self.min_child_number_spinbox)
        ...
        self.layout.addRow(QLabelWithTooltip("Maximum depth:"), self.max_depth_spinbox)
        ...
        self.layout.addRow(QLabelWithTooltip("Minimum metrics change:"), self.min_metrics_spinbox)

        self.metrics_type_box = QComboBox()
        self.metrics_type_box.addItems(['gini', 'entropy'])
        self.layout.addRow(QLabelWithTooltip("Metrics type:"), self.metrics_type_box)

    def get_data(self) -> dict:
        max_depth = self.max_depth_spinbox.value() if self.max_depth_spinbox.value() > 1 else None
        return {
