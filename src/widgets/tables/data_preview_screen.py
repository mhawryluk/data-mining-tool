...
class DataPreviewScreen(QWidget):
    def __init__(
        ...
        widget,
        title="Data preview",
        reason: PreviewReason = PreviewReason.PREVIEW,
    ):
        super().__init__()
        self.parent = widget
        ...
        match reason:
            case PreviewReason.ESTIMATION:
                self.render_instruction(
                    "Double click on a cell to fill in a missing value or on a header to affect "
                    "the entire column"
                )
                self.render_data(
                    self.estimation_header_click, self.estimation_cell_click
                )
            case PreviewReason.REDUCTION:
                self.render_instruction(
                    "Double click on a header to change column name"
                )
    ...
    def render_data(self, handle_header_click, handle_cell_click):
        ...
        self.data_table.horizontalHeader().sectionDoubleClicked.connect(
            handle_header_click
        )
    ...
    def estimation_header_click(self, index):
        new_value, ok = QInputDialog.getText(
            self, f"Default value for column {index}:", "Value:", QLineEdit.Normal, ""
        )
        if ok:
