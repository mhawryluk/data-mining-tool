...
from utils import AutomateSteps, QImage, QtImageViewer
...
class TreeStepsVisualization(QWidget):
    def __init__(
        self, widget: QWidget, node_info: Dict, dot_steps: List[str], is_animation: bool
    ):
        super().__init__()
...
class ExtraTreesStepsVisualization(AlgorithmStepsVisualization):
    def __init__(
        self,
        data: pd.DataFrame,
        algorithms_steps: List[Tuple[str, Dict, List[str]]],
        is_animation: bool,
    ):
        ...
        self.layout = QVBoxLayout(self)
        self.graphs = [
            self.make_graph(dot_string) for dot_string, _, _ in self.algorithms_steps
        ]
    ...
    def make_graph(self, data: str):
        ...
        poses = [
            float(node.attr["pos"].split(",")[1]) for node in graph_info.nodes_iter()
        ]
        ...
        qgv.new(
            Dot(
                Graph("graph", graph_type=GraphType.DirectedGraph),
        ...
        for line in lines:
            ...
            if len(value_list) == 1:
                ...
                param_dict["pos"] = [
                    float(value)
                    for value in graph_info.get_node(node).attr["pos"].split(",")
                ]
    ...
    def click_listener(self, button_type: str):
        match button_type:
            case "steps":
                ...
                self.steps_window = TreeStepsVisualization(
                    self, data[1], data[2], self.is_animation
                )
