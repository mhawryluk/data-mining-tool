...
class APrioriStepsVisualization(AlgorithmStepsVisualization):
    def __init__(
        self, data: pd.DataFrame, algorithms_steps: List[dict], is_animation: bool
    ):
        ...
        self.visualization_box_layout.addWidget(
            self.algorithm_part_label, 0, alignment=Qt.AlignCenter
        )
    ...
    def _render_control_ui(self):
        if self.is_animation:
            self.automat = AutomateSteps(
                lambda: self.change_step(1),
                lambda: self.change_step(-1 * self.current_step),
            )
            self.is_running = False
            ...
            self.next_part_button.clicked.connect(
                partial(self.click_listener, "next part")
            )
            self.prev_part_button = QPushButton("PREV PART")
            self.prev_part_button.clicked.connect(
                partial(self.click_listener, "prev part")
            )
    ...
    def click_listener(self, button_type: str):
        match button_type:
            ...
            case "next part":
                ...
                while (
                    self.current_step + step_delta <= self.max_step
                    and self.algorithms_steps[self.current_step + step_delta]["part"]
                    == current_part
                ):
                    step_delta += 1
            ...
            case "prev part":
                ...
                current_part = self.algorithms_steps[self.current_step]["part"]
                while (
                    self.current_step + step_delta >= 0
                    and self.algorithms_steps[self.current_step + step_delta]["part"]
                    == current_part
                ):
                    step_delta -= 1

                current_part = self.algorithms_steps[self.current_step + step_delta][
                    "part"
                ]
                while (
                    self.current_step + step_delta >= 0
        ...
        match step_dict["part"]:
            case APrioriPartLabel.CALCULATE_SUPPORT:
                description = (
                    ...
                    "Its support equals {} \nIt is {}a frequent set.".format(
                        format_set(step_dict["set"]),
                        round(step_dict["support"], 3),
                        "not "
                        if step_dict["support"] < step_dict["min_support"]
                        else "",
                    )
                )
                self.gauge_chart.plot_value(
                    round(step_dict["support"], 3), step_dict["min_support"], "support"
                )
                self.graph_plot.plot_set(step_dict["set"])
            case APrioriPartLabel.FILTER_BY_SUPPORT:
                description = (
                    "We have found that the following sets "
            ...
            case APrioriPartLabel.SAVE_K_SETS:
                description = "We have found all frequent sets for k={}".format(
                    step_dict["k"]
                )
            case APrioriPartLabel.SAVE_RULES:
                description = (
                    "We have found all association rules for "
                    "specified minimum confidence and support."
                )
            case APrioriPartLabel.JOIN_AND_PRUNE:
                description = (
                    "We are joining sets: {} and {}, "
                    "then analyzing resulting set: {}. ".format(
                        format_set(step_dict["set_1"]),
                        format_set(step_dict["set_2"]),
                        format_set(step_dict["new_set"]),
                    )
                )

                if step_dict["infrequent_subset"] is None:
                    description += (
                        "This set does not contain any infrequent "
                        "subsets.  It might be frequent itself."
                    )
                else:
                    description += (
                        "This set contains an infrequent subset: "
                        "{}. Therefore it is not frequent itself.".format(
            ...
            case APrioriPartLabel.GENERATE_RULES:
                description = (
                    "We divide frequent set into A = {} and B = {}. "
                    "The confidence of the rule A => B "
                    "equals {}\n\n".format(
                        format_set(step_dict["set_a"]),
                        format_set(step_dict["set_b"]),
                ...
                if step_dict["confidence"] >= step_dict["min_confidence"]:
                    ...
                    description += (
                        "It is not enough to consider it a "
                        "valid association rule for our data."
                    )

                self.graph_plot.plot_rule(step_dict["set_a"], step_dict["set_b"])
                self.gauge_chart.plot_value(
                    round(step_dict["confidence"], 3),
                    step_dict["min_confidence"],
                    "confidence",
                )
